...
if __name__ == "__main__":
    ...
    parser.add_argument("-points", "--points", help="toa do diem dau, diem cuoi")
    parser.add_argument("-omg", "--omega", type=str, help="path file omega")
    parser.add_argument("-out", "--out", type=str, help="path file output")
    ...
    y0=p[:,0]
    goal=p[:,1]
    omega_input = np.loadtxt(args.omega).reshape(int(args.n_dmps),10000)
    omega_input = np.array(omega_input, ndmin=2)
    dmp = DMPs_discrete(n_dmps=int(args.n_dmps), n_bfs=10000, dt=1 /200)
    dmp.y0 = y0

    dmp.goal =goal
    dmp.w = omega_input
    q_track, dq_track, ddq_track = dmp.rollout()
    q_target = np.array(q_track)
...
#     plt.show()
#     import scipy.io
#     scipy.io.savemat('E_rang3_dmps.mat', q_target)
    np.savetxt(args.out,q_target)
#     return q_target
